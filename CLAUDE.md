...
- **沉浸光感 API 已确认**（见 `docs/notes-immersive-light-sense.md`）：`@kit.UIDesignKit` 的 `systemMaterialEffect`，施加于 `HdsNavigation` 标题栏与 `HdsTabs` 底部页签；地基 Task 12 用 `backgroundBlurStyle` 占位，真机 D7 用 HDS 组件实现真实光感。
...
| `docs/plan-p1-device-integration-2026-06-23.md` | **P1 真机集成计划**（D1–D10，需 AGC 云调试真机） |
| `docs/notes-immersive-light-sense.md` | **沉浸光感 API 确认笔记**（`systemMaterialEffect` / HDS 组件 / 降级策略） |
| `docs/reference-deveco-config.md` | **DevEco 配置参考**（Task 1 脚手架后合并：权限/NDK/依赖） |
...
| `pending-src/` | **暂存源码**（地基 Task 2-3 已写好，Task 1 建工程后按其 README 贴入） |
| **GitHub 远程** | `https://github.com/zsnmfj-tech/harmonyos-spatial-commerce`（Private，分支 `main`） |
...
- 沉浸光感（ArkTS）：https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-immersive-light-sense （原始页抓取不稳；API 已从 HDS 材质文档确认，见 `docs/notes-immersive-light-sense.md`）
...
2. 说一句：「继续 harmonyos-spatial-commerce 项目」（或更具体：「继续执行 P1 计划的 Task N」）。
...
2. ~~沉浸光感在 XComponent 覆盖层上的真实表现~~ → **已澄清**：光感官方施加点是 `HdsNavigation`/`HdsTabs`（外围 chrome），非 XComponent 覆盖层；走"外围 chrome"分支，见 `docs/notes-immersive-light-sense.md`。
